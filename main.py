...
for key in ticker_dict:
    ...
    print(f"prev_day: {prev_day}, today: {today}, next_day: {next_day}, tickers: {tickers}")
    df = pd.DataFrame()
    for ticker in tickers:
        ...
        if prev_day:
            prev_day_output = convert_YYYY_MM_DD_to_MM_DD_YYYY(prev_day)
        if today:
            today_output = convert_YYYY_MM_DD_to_MM_DD_YYYY(today)
        if list_date:
            list_date_output = convert_YYYY_MM_DD_to_MM_DD_YYYY(list_date)
        if next_day:
            next_day_output = convert_YYYY_MM_DD_to_MM_DD_YYYY(next_day)
    ...
    if os.path.exists(filename):
        with pd.ExcelWriter(filename, engine='openpyxl', mode='a') as writer:
            ...
            if today_output in writer.book.sheetnames:
                idx = writer.book.sheetnames.index(today_output)
                writer.book.remove(writer.book.worksheets[idx])
            df.to_excel(writer, sheet_name=today_output, index=False)
    else:
        df.to_excel(filename, sheet_name=today_output, index=False)
    ...
    #read master csv file and drop duplicates based on ticker, today, prev_day,next_day and keep the last row
    df = pd.read_csv(master_filename)
    df = df.drop_duplicates(subset=['ticker', 'today', 'prev_day','next_day'], keep='last')
